...
@equinox.filter_jit
def simulate_trajectory_mppi_td(mx, dx, set_control_fn, running_cost_fn, terminal_cost_fn, value_net, U, gamma = 1.0, td=50):
    ...
    td = min(td, len(U))
    U_td = U[:(td+1)]
    jax.debug.print("dx: {}", dx.qpos)
    (dx_final, H), (states, costs) = jax.lax.scan(step_fn, (dx, 0), U_td)
    jax.debug.print("dx_final: {}", dx_final.qpos)
    state_final = jnp.concatenate([dx_final.qpos, dx_final.qvel])
    total_cost = jnp.sum(costs) + value_net(state_final) * (gamma ** len(U_td))
    return states, total_cost
...
class POLO:
    ...
    def mppi_solver(self, dx, U, key, t):
        ...
        simulate_trajectory_batch = jax.vmap(simulate_trajectory_mppi_td, in_axes=(None, None, None, None, None, None, 0))
    ...
    def mppi_target(self, state, U, key, U_opt=None):
        ...
        _, target_cost = simulate_trajectory_mppi_td(mx, dx_internal, set_control, running_cost, terminal_cost, value_net, optimal_U)
...
if __name__ == "__main__":
    ...
    update_frequency, mini_batch, grad_steps = 20,20,10
