...
def get_model_config(xml_name):
    """
    Retrieve the configuration for a specific model using a match statement.

    Args:
        xml_name: Name of the XML file.

    Returns:
        A tuple of (qpos_init, terminal_cost_fn).
    """
    match xml_name:
        case "cartpole.xml":
            qpos_init = jnp.array([0.0, 3.14])
            terminal_cost_fn = lambda dx: (dx.qpos[0] ** 2 + (jnp.sin(dx.qpos[1] / 2) * jnp.pi) ** 2) * 1.0
        case "finger_mjx.xml":
            qpos_init = jnp.array([0.0, 0.0, jnp.pi/2])
            terminal_cost_fn = lambda dx: ((jnp.mod(jnp.abs(dx.qpos[2]), jnp.pi)) ** 2) * 1.0
        case _:
            raise ValueError(f"Unknown XML model: {xml_name}")
    
    return qpos_init, terminal_cost_fn


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--xml", type=str, default="cartpole.xml")
    ...
    if args.vis >= args.batch:
        parser.error("The --vis argument must be smaller than the --batch argument.")

    path = "xmls/" + args.xml
    model = mujoco.MjModel.from_xml_path(path)
    mx = mjx.put_model(model)
    dx = mjx.make_data(mx)
    Nsteps, nu, N_rollouts, batch_size = args.nsteps, mx.nu, args.nrolls, args.batch

    # here
    # qpos_init = jnp.array([0.0, 3.14])
    qpos_init, terminal_cost = get_model_config(args.xml)
    ...
    optimizer = MPPI(loss=loss_fn, grad_loss=grad_loss_fn, lam=args.lambd, U_init=U, running_cost=running_cost, terminal_cost=terminal_cost, set_control=set_control, mx=mx)
