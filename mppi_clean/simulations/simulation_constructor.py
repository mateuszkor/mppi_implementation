...
class SimulationConstructor:
    ...
    def create_simulation(sim_name: str, config: Dict[str, Any], mx: mjx.Data) -> Tuple[
        ...
        if sim_name == "cartpole":
            from simulations.cartpole import cartpole_costs, termination_function
            qpos_init = jnp.array([0.0, 3.14])
            return qpos_init, termination_function, *cartpole_costs(config)
        elif sim_name == "hand_fixed":
            from simulations.hand_fixed import generate_qpos_init, hand_fixed_costs, termination_function 
            qpos_init = generate_qpos_init(config['hand']['qpos_init'], mx)
            term_func = termination_function(config['hand']['goal_quat'])
            return qpos_init, term_func, *hand_fixed_costs(config)
